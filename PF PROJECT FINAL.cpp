...
typedef struct
{                  // structure to store the information of passengers
...
void displayStops()
{ // displays all the available stops
    printf("\nAvailable Stops:\n");
...
{                        // stopIndex is the current stop
    ...
    do
    {
        ...
        { // Check if the user selects the current stop
            printf("Invalid stop selection. The ending stop cannot be the current stop.\n");
        }
        else if (endChoice < 1 || endChoice > MAX_STOPS)
        { // Ensure stop is within valid range
...
{ // used pointer for discountDetails b\c it will be updated and used in main
...
{ // to check for passengers at each stop
    char choice;
    ...
    do
    {
        ...
        { // if choice = y or Y then again display the book ticket function
...
void viewloyaltyprofiles()
{ // to display loyal customers data
...
void displayscreen()
{ // function to display welcome screen consisting of system name and a bus icon
    printf("\n");
    printf("                                      \n");
...
int main()
{
    ...
    do
    {
        ...
        case 1: // if choice is "Boarding a ticket"
            for (int i = 0; i < MAX_STOPS; i++)
            {                               // to iterate through each stop i.e 5
